...
Чому вигляд описаний саме так. У CRM емоція виражається чотирма
каналами: очі, антена, руки, ноги. У ROBI з них лишається **один** —
кінцівки статичні (`D-019`), антена не має світлодіода (`D-045`), а
канал RGB знято (`D-043`). Тому кожен стан переведено в те, що
пристрій справді вміє: форма очей, зміщення погляду й іскри.

Це не збіднення образу, а чесне його перенесення: візуальний гайд
бренду прямо називає очі «основним емоційним елементом», і саме вони
лишаються.
"""
...
LOOKS: dict[MascotState, Look] = {
    ...
    # «закриті / обертаються» — обертання ROBI не малює, лишаються закриті.
    ...
    # «сльозинка / опущені» — сльози ROBI не малює, лишається погляд донизу.
