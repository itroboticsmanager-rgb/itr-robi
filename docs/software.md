...
Стек для vertical slice — Python 3.11+ і pygame-ce (SDL2), статус `provisional`: остаточний вибір за `D-015` робить бенчмарк, а не вподобання. Рендерер тримається за інтерфейсом, тож заміні підлягає він, а не застосунок. База/сховище й система пакування **не обрані**. Вибір слід робити після короткого proof of concept на цільовій платформі з цільовою роздільністю дисплея. Розробку можна починати на ПК проти fake hardware — це не блокується вибором заліза (`D-023`).
